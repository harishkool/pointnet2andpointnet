...
class Mesh:
    ...
    def plot_both(self, c1='m', c2='k'):
        ...
        if p:
            ax.add_collection3d(Poly3DCollection(p))
        if v.shape:
            ax.scatter(v[:, 0], v[:, 1], v[:, 2], c=c1)
        if pc is not None:
            ax.scatter(pc[:,0], pc[:,1], pc[:,2], c=c2)
        return fig, ax
    ...
    def pc_reader(self, filepath,np_array=False):
        if np_array:
            self._points = np.load(filepath)
        else:
            self._points = pd.read_csv(filepath, header=None, sep=' ').values
...
if __name__ == '__main__':
    def test1():
        ...
        parser.add_argument('--np_array', default=False, help='True if the input is numpy array')
        args = parser.parse_args()
        mesh = objread('model_normalized.obj', points_only=False)
        mesh.pc_reader('../test_transform_withoutbias_9000.pts', args.np_array)
        #mesh.on_unit_sphere()
        # mesh.rot_x()
        mesh.plot_both()
        # mesh.plot(c='m')
